...
public class WorkService {
    ...
    public List selectPage(String userId, String userName, int index, int pageSize) {
        Page<Work> page = new Page<>(index,pageSize);
        return workMapper.selectPage(page,new QueryWrapper<Work>().lambda().eq(StringUtil.isNotEmpty(userId),Work::getUserId, userId)
                .like(StringUtil.isNotEmpty(userName), Work::getUserName, userName).orderByDesc(Work::getCreateTime)).getRecords();
    }
